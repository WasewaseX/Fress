  ...
  const generateScript = () => {
    ...
    if (scriptType === 'brewfile') {
      const lines = selectedApps
        .filter((a) => a.brewCommand)
        .map((a) => {
          const pkg = a.brewCommand?.replace('brew install --cask ', '').replace('brew install ', '');
          return `cask "${pkg}"`;
        });
    ...
    if (scriptType === 'flatpak') {
      const commands = selectedApps
        .filter((a) => a.flatpakCommand)
        .map((a) => `${a.flatpakCommand} -y`);
    ...
    if (scriptType === 'scoop') {
      const pkgs = selectedApps
        .filter((a) => a.scoopCommand)
        .map((a) => a.scoopCommand?.replace('scoop install ', ''))
        .join(' ');
      ...
      >
        ...
        <div className="px-5 py-3.5 bg-slate-950 border-t border-slate-950/10 dark:border-white/[0.08] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <span>All commands pull directly from official signed manifests.</span>
          </div>
